document.addEventListener('DOMContentLoaded', function() {
    ...
    {"id":95,"comboName":"強P~強P>ラッシュ屈強P>中スピン~弱リンクス~強化ハーベスト>弱昇竜>弱昇竜","starter":"強P","hitCondition":"パニッシュカウンター","targetState":"指定なし","position":"画面端","saCost":0,"driveCost":1,"damage":4081,"saGain":0.66,"driveGain":0.9,"carry":1,"okizeme":5,"difficulty":3,"notes":""},
    ...
    {"id":245,"comboName":"屈中P>DR>屈強P>ODリンク~強化ハーベスト>SA2","starter":"屈中P","hitCondition":"ノーマル","targetState":"指定なし","position":"指定なし","saCost":2,"driveCost":6,"damage":3812,"plusFrame":0,"saGain":0.27,"driveGain":0.2,"plusFrame":25,"carry":4,"okizeme":3,"difficulty":2,"notes":"4本から可能"},
    ...
    function getSortKeys(starterName) {
        ...
        else if (starterName.match(/^[0-9]/)) prefix = 4;
    ...
    function populateFilters() {
        const createOptions = (selectElement, key) => {
            // HTMLにあらかじめ定義されている基本の選択肢は保持する
            const baseOptions = Array.from(selectElement.options).map(opt => opt.value);
            
            // データから新しい選択肢を抽出
            const newItems = [...new Set(comboData.map(combo => combo[key]))]
                .filter(item => item && !baseOptions.includes(item)); // 空でなく、まだ存在しないもの
    ...
    resetButton.addEventListener('click', () => {
        ...
        resultsBody.innerHTML = '<tr><td colspan="7" class="center-align" style="padding: 20px;">上の条件を設定して「コンボを検索」ボタンを押してください。</td></tr>';
    });

    searchButton.addEventListener('click', function() { runAnalysis(); });
    ...
    function displayResults(data) {
        ...
        if (!data || data.length === 0) { resultsBody.innerHTML = '<tr><td colspan="7" class="center-align">該当するコンボが見つかりませんでした。</td></tr>'; return; }
